...
import com.protoplus.newscircle.newscircle.MainContent.TechCrunchMainContent;
 ...
 */
public class TechCrunchNewsFragment extends Fragment implements SwipeRefreshLayout.OnRefreshListener,IGetNotification {
    int counter=0;
    int Minimum_Value=0;
    private String category;
    int position;
    private final int web_index = 24;
                ...
                parser.parse(urlString, new DefaultHandler(){
                                             ...
                                             String qName, Attributes attributes) throws SAXException
                    {
                        if (qName.equals("item")){
                            ...
                            String s= attributes.getValue("url");

                            if(s.contains(".png")){
                                s=s.substring(s.indexOf("http"),s.indexOf(".png")+4);
                            }else if(s.contains(".jpg")){
                                s=s.substring(s.indexOf("http"),s.indexOf(".jpg")+4);
                            }
                            if(!s.contains("https") && s.length()>20){
                                s=s.replace("http","https");
                            }
                            if(s.length()>20){
                                System.out.println(s + "?w=210");
                                bean.setImageLink(s + "?w=210");
                                if(!s.contains("htt?")){
                                    bean.setMainImageLink(s);
                                }
                            }
                        }
                    }
                        ...
                        if (qName.equals("item") && bean!=null)
                        {
                            if(Helper.isAppOpen){
                                if(bean.getTitle().equals(ar.get(0).getTitle())){
                                    ...
                                    bean.setMainContentClass(TechCrunchMainContent.class.getName());
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        listView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> arg0, View arg1, int arg2,
                                    long arg3) {
                Intent it = new Intent(getActivity().getApplicationContext(), TechCrunchMainContent.class);
                ContextData.setTechCrunchNewsFragment_bean_array(ar);
