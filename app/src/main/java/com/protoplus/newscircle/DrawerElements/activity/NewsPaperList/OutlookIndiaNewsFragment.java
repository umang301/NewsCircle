...
import com.protoplus.newscircle.newscircle.MainContent.OutLookIndiaMainContent;
...
public class OutlookIndiaNewsFragment extends Fragment implements SwipeRefreshLayout.OnRefreshListener,IGetNotification {

    int counter=0;
    private String category;
    int Minimum_Value=0;
    int position;String pubDateText = "";
    private final int web_index = 22;
                        ...
                        if (qName.equals("item") && bean!=null)
                        {
                            if(Helper.isAppOpen){
                                if(bean.getTitle().equals(ar.get(0).getTitle())){
                                    System.out.println("----------------refresh data : position : " + position + " the size of arraylist is : " + ar.size());
                                    ContextData.getMain_ar().set(position, ar);
                                    throw new SAXException();
                                }
                                else  if(bean!=null){
                                    bean.setMainContentClass(OutLookIndiaMainContent.class.getName());
                            ...
                            throws SAXException
                    {
                        if (title  && bean!=null){
                            ...
                            if(descriptionText.contains("http:") && descriptionText.contains(".jpg")){
                                String ImageLink = descriptionText.substring(descriptionText.indexOf("http:"),descriptionText.indexOf(".jpg")+4);
                                System.out.println("THe imagelink is :::: "+ImageLink);
                                ImageLink.replace("http:","https:");
                                bean.setImageLink(ImageLink);
                            }
...
/*==================================================================*/
    ...
    public OutlookIndiaNewsFragment(){
        if(Helper.isAppOpen){
            urlString=Helper.getCurrentURL();
            System.out.println("outlookindia url :"+urlString);
            position=ContextData.getPosition();
            System.out.println("-----------------I am In OneIndia Fragment Constructer : POSITION"+position);
            try{
            ...
            System.out.println("-----------------I am in OneIndia Fragment constructer : size of Arraylist : "+ar.size());
    ...
    private class NewTab extends AsyncTask<String,String,String>{
        @Override
        protected String doInBackground(String... params) {
            System.out.println("I am in do in background oneindia news fragment");
            new NewTabData();
            return "";
        }
        @Override
            ...
            if (qName.equals("item") && bean!=null)
            {
                ...
                if(counter>=6){
                    ...
                    for(SportChildSceneBean bean:ar){
                        System.out.println("POSITION:"+position+ " array index:"+ar_index++ +". Title : "+bean.getTitle());

                    }
                    System.out.println("----------------onScroll data : position : " + position + " the size of arraylist is : " + ar.size());
                    ContextData.getMain_ar().set(position, ar);
                    throw new SAXException();
                }else{
                    counter++;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        listView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> arg0, View arg1, int arg2,
                                    long arg3) {
                Intent it = new Intent(getActivity().getApplicationContext(), OutLookIndiaMainContent.class);
                ContextData.setOutLookIndia_bean_array(ar);
